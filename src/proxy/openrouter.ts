...
const getModelsResponse = async () => {
  ...
  try {
    ...
    const modelToUse = "deepseek/deepseek-chat-v3.1:free"; // Use any XAI model here - just for key selection
...
const handleImageGenerationRequest: RequestHandler = async (req, res) => {
  try {
    ...
    const modelToUse = req.body.model || "google/gemini-2.5-flash-image-preview"; // Default model
    const openrouterKey = keyPool.get(modelToUse, "openrouter") as OpenrouterKey;
